...
function fakeBridge(overrides: Partial<HostBridge> = {}): HostBridge & { calls: [string, unknown[]][]; logs: string[] } {
  ...
  return {
    ...
    self: rec("self", { id: "n0", q: 0, r: 0, stomach: 42 }),
    move: rec("move", true),
...
describe("sandbox: nothing from the host is reachable", () => {
  ...
  test("me exposes the live body next to set()", async () => {
    const { sb } = await mk();
    expect(sb.eval("me.stomach < 50 && me.id === 'n0'")).toMatchObject({ ok: true, value: "true" });
    expect(sb.eval("me.stomach = 1; me.stomach")).toMatchObject({ ok: true, value: "42" });
  });
...
describe("sandbox: bridge semantics", () => {
  test("every host function is exposed through the prelude API", async () => {
    ...
    const r = sb.eval(`
      ...
      ruins.files("n9"); ruins.read("n9", "main.js"); me.set("group", "river"); me.stomach; log("done", {b: 2});
    `);
    expect(r.ok).toBe(true);

...
export class OllamaBrain implements Brain {
  ...
  async decide(req: DecisionRequest, opts: DecideOptions = {}): Promise<DecisionResult> {
    ...
    const body = {
      ...
      stream: this.stream,
      options: { temperature: req.temperature ?? this.temperature, num_predict: req.maxTokens ?? this.maxTokens },
    };
    ...
    return { text, latencyMs, tokens: n, tokensPerSec: tps, estimated };
  }
